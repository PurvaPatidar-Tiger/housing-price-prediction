...
def create_job_id(context):
    ...
    Parameters
    ----------
    context : housing.core.context.Context
...
def get_package_path():
    """Get the path of the current installed housing package.

    Returns
    -------
    str
        path string in the current system where the housing package is loaded from
    """
    path = housing.__path__
    return op.dirname(op.abspath(path[0]))


def get_package_version():
    """Return the version of the package."""
    return housing.__version__
...
def get_feature_names_from_column_transformer(col_trans):
    ...
    for (
        ...
        if is_pipeline:
            ...
            names = [f"{transformer_in_columns[0]}_{col_}" for col_ in names]+['num_rooms_per_household', 'num_population_per_household','num_bedrooms_per_room']
