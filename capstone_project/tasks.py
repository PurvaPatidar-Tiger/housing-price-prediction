...
NOTEBOOK_FOLDER = op.join(HERE, "notebooks", "reference")
...
def _build_docker_image(c):
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        shutil.copytree(op.join(HERE, "src", "housing"), op.join(tempdir, "src", "housing"))
...
def run_unit_tests(c, platform=PLATFORM, env=DEV_ENV, markers=None):
    ...
    with py_env(c, env_name):
        ...
        c.run(f"python -m pytest -v {TESTS_FOLDER} {markers}")
...
@task(name="randon_implementation")
def implement_radon(c, platform=PLATFORM, env=DEV_ENV):
    env_name = _get_env_name(platform, env)
    with py_env(c, env_name):
        c.run(f"python -m radon cc -i {HERE}/production/conf {HERE}/production ")
